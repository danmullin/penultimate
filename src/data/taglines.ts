/**
 * Rotating brand taglines under PENULTIMATE (brother-facing — keep it clean).
 * Private Mist lines live in MEMORY / chat, not here.
 */
export const TAGLINES: string[] = [
  // Canonical
  'Handles worth dragging',
  'The last pen you’ll need (until the next one)',

  // Same joke, new punchlines
  'One last Bézier. We mean it this time.',
  'Closing the path on “just one more tool”',

  // Pen / path
  'Click once. Drag forever.',
  'Anchor management, rebranded',
  'Join endpoints, not meetings',
  'Smooth enough to lie about',
  'Close the path. Keep the options.',
  'Anchor points have feelings too',
  'Convert anchor. Convert yourself.',
  'Scale from center. Scale from within.',
  'Clipping mask of confidence',
  'Compound path, compound interest',

  // Tool jokes
  'Direct select your problems',
  'Eyedropper of truth',
  'Group therapy for overlapping shapes',
  'Layers so deep we need a map',
  'Marquee like you mean it',
  'Pen tool with commitment issues',
  'Shear pressure. Emotional too.',

  // Graphic design
  'Kerning: the silent treatment',
  'Negative space, positive attitude',
  'CMYK my way through this',
  'Pantone of my problems',
  'Contrast is a love language',
  'White space is doing the work',
  'Align left. Feel right.',
  'Rule of thirds. Rule of cool.',
  'Mockup energy, final-file anxiety',
  'The client said “make it pop”',
  'Version 47_FINAL_final2',

  // Typography
  'Ligatures for life',
  'Baseline jump scare',
  'Small caps, big feelings',
  'Em dash — my favorite punctuation',
  'Italicize the vibe',
  'Bold claims, regular delivery',

  // Nerdy-cute
  'Undo is forever. Commitment is Ctrl+S.',
  'Pathfinder of least resistance',
  'Live preview of good decisions',
  'Snap to what matters',
  'Curve appeal.',
  'Make it, then make it again',
  'Soft save. Hard opinions.',
  'Still not Illustrator. On purpose.',
  '1px stroke, infinite personality',
  'Bounding box, unbound ambition',
  'Hit Enter to finish. Or don’t.',
  'Pink is a strategy.',
  'No native color pickers were harmed',

  // Corporate satire (Anchor Management cousin)
  'Enterprise-grade doodling since just now',
  'Your bezier liaison',
  'Stroke compliance, voluntarily',
  'Fill rates may vary',
  'Now with 12% more “I’ll fix it in post”',
  'Synergizing paths since you opened the tab',
  'KPI: cute shapes shipped',
  'Ideating rectangles at scale',
  'Roadmap: more handles',
  'Stakeholder-approved curves',
  'Syncing swatches across feelings',
  'Non-blocking UI. Blocking opinions.',

  // Bad tips (veteran laughs)
  'Pro tip: never name your layers',
  'Save as Final. Then Final2. Then Final2_real',
  'When in doubt, add a drop shadow',
  'Rasterize it. The future is now.',
  'Just eyeball the kerning',
  'Center it. Then center it again.',
  'Use the default swatches. Be free.',
  'Lock the wrong layer. Character building.',
  'Duplicate before you think',
  'Undo until it feels like art',
  'Zoom to 6400% for confidence',
  'If it looks wrong, add another stroke',
  'Align to whatever’s closest',
  'Hold Alt. Something good will happen.',
  'Embed the fonts. All of them. Forever.',
  'RGB for print. Live a little.',
  'Pen tool: more points = more better',
  'Smooth it until it’s a potato',
  'Expand appearance. Expand your mind.',
  'Clipping mask everything you fear',
  'Send a PDF of a screenshot of a PDF',
  'I’ll know it when I see it (the brief)',
  'Don’t group. Live dangerously.',
  'Comic Sans, but ironic.',

  // Short sticky
  'Penultimate means we meant it',
  'Fewer tabs. More handles.',
  'Fit artboard. Fit narrative.',
  'Make it kern.',
  'Less Comic Sans.',
  'Snap to baseline.',
]
